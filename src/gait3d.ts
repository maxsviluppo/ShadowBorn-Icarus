...
export function solveLeg(phase:number,side:'L'|'R',blend:number){
  const t=((phase/(Math.PI*2)+(side==='L'?0:.5))%1+1)%1;
  const swing=t>=.5,p=swing?(t-.5)*2:t*2;
  const z=(swing?-.20+.4*p*p*(3-2*p):.20-.4*p)*blend;
  const lift=(swing?Math.sin(p*Math.PI)*.095:0)*blend;
  const y=.51-lift+Math.cos(phase*2)*.005*blend;
...
export function advanceGait(phase:number,distance:number){
  const next=phase+Math.max(0,distance)/STRIDE_METRES*Math.PI*2;
