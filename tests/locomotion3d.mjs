...
assert.ok(Math.abs(WALK_SPEED-.715)<1e-10);
...
for(const hz of [24,30,60,120])for(const run of [false,true]){
 ...
 assert.ok(r.drop<2.5/hz+.035,'no abrupt stop');
...
console.log('PASS: walk +10%, run promotion, bounded braking, escape, rerouting, collision safety at 24/30/60/120 Hz.');
